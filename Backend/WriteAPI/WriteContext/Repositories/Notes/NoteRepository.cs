...
namespace WriteContext.Repositories.Notes
{
    public class NoteRepository : Repository<Note>
    {
        ...
        public async Task<List<Note>> GetNotesByUserIdAndTypeIdWithContent(Guid userId, Guid typeId, bool withHistory)
        {
            return await context.Notes
                .Include(x => x.RefType)
                .Include(x => x.NoteType)
                ...
                .OrderBy(x => x.Order)
                .Where(x => x.UserId == userId && x.NoteTypeId == typeId && x.IsHistory == withHistory).ToListAsync();
        }
