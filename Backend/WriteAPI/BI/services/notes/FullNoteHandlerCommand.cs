        ...
        IRequestHandler<InsertFilesToNoteCommand, OperationResult<DocumentNoteDTO>>
    {
        ...
        public async Task<OperationResult<AlbumNoteDTO>> Handle(InsertAlbumToNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            if (permissions.CanWrite)
            {
                ...
                // FILES LOGIC 
                var fileList = await _mediator.Send(new SavePhotosToNoteCommand(permissions.User.Id.ToString(), request.Photos, note.Id));
        ...
        public async Task<OperationResult<List<Guid>>> Handle(UploadPhotosToAlbum request, CancellationToken cancellationToken)
        {
            ...
            if (permissions.CanWrite)
            {
                ...
                var fileList = await this._mediator.Send(new SavePhotosToNoteCommand(permissions.User.Id.ToString(), request.Photos, note.Id));
            ...
            if (permissions.CanWrite)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        await _mediator.Send(new RemoveFilesByPathesCommand(permissions.User.Id.ToString(), new List<string> { photoForRemove.Path }));
        ...
        public async Task<OperationResult<AudioNoteDTO>> Handle(InsertAudiosToNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            if (permissions.CanWrite)
            {
                ...
                // FILES LOGIC
                var file = await _mediator.Send(new SaveAudiosToNoteCommand(permissions.User.Id.ToString(), request.Audio, note.Id));
        ...
        public async Task<OperationResult<VideoNoteDTO>> Handle(InsertVideosToNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            if (permissions.CanWrite)
            {
                ...
                // FILES LOGIC
                var file = await _mediator.Send(new SaveVideosToNoteCommand(permissions.User.Id.ToString(), request.Video, note.Id));
        ...
        public async Task<OperationResult<DocumentNoteDTO>> Handle(InsertFilesToNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            if (permissions.CanWrite)
            {
                ...
                // FILES LOGIC
                var file = await _mediator.Send(new SaveDocumentsToNoteCommand(permissions.User.Id.ToString(), request.File, note.Id));
