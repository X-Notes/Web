...
export class HtmlTitleService {
  ...
  init = () => {
    this.store.select(AppStore.getRouting).subscribe((route) => this.onRouteChange(route));
    this.store.select(UserStore.getUserLanguage).subscribe(() => {
  ...
  private onRouteChange(type: EntityType) {
    ...
    switch (type) {
      ...
      case EntityType.FAQ: {
        this.titleService.setTitle(this.translateService.instant('FAQ'));
        break;
      }
