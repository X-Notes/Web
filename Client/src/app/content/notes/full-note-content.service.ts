...
export class FullNoteContentService {
  ...
  getContent(): ContentModel[] { // TODO REMOVE
    ...
    content1.data = {
      html: `<p>TypeScript реализует <b>объектно-ориентированный <i>подход, в нем <h1>есть полноценная поддержка</h1> классов. Класс</i> представляет шаблон
      для создания объектов и инкапсулирует функциональность, которую должен иметь объект.</b> Класс определяет состояние и поведение, которыми обладает объект.</p>`
    };
    ...
    content2.data = {
      html: `<p>
      <b>TypeScript реализует</b> объектно-ориентированный подход, в нем есть полноценная поддержка классов. Класс представляет шаблон
для создания объектов и инкапсулирует функциональность, которую должен иметь объект. Класс определяет состояние и поведение, <b>которыми обладает объект.</b>
       </p>
       <p>
      <b>TypeScript реализует</b> объектно-ориентированный подход, в нем есть полноценная поддержка классов. Класс представляет шаблон
для создания объектов и инкапсулирует функциональность, которую должен иметь объект. Класс определяет состояние и поведение, <b>которыми обладает объект.</b>
